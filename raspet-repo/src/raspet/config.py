...
KEYPAD_BACK_KEY = "D"     # 게임 중 이 키 = 뒤로/종료
...
# DHT11(온도+습도) — 단선 디지털(1-wire 방식). I2C가 아니라 GPIO 1개를 쓴다.
# ※ BMP180과 달리 기압은 못 재지만 습도를 잰다. 정밀도는 낮다(온도 ±2℃·습도 ±5%, 정수 단위).
DHT11_GPIO = 19            # 데이터 핀(BCM). 물리핀 35. 옆 핀39=GND, 전원은 3.3V 레일에서.
DHT11_RETRIES = 5          # 단발 읽기 실패(체크섬·타이밍 어긋남) 시 재시도 횟수
# 온도 쾌적 범위(℃). 이 범위를 벗어나면 캐릭터가 더워/추워한다.
TEMP_COLD_BELOW_C = 15      # 이하 → 추워요
TEMP_HOT_ABOVE_C = 28       # 이상 → 더워요
# 습도(%) — DHT11로 측정한다. 이 값 이상이면 펫이 '끈적해요'.
HUMID_HIGH_ABOVE = 70       # % 이상 → 끈적해요
