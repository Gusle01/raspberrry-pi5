...
| 4×4 키패드 | 행(출력) | GPIO4·17·27·22 (핀 7·11·13·15) | `PIN_KEYPAD_ROWS` |
...
### 3-3. 4×4 매트릭스 키패드
행 4개(GPIO4·17·27·22)는 출력, 열 4개(GPIO12·16·20·21)는 입력(내부 풀업)입니다.
...
- 키 배치는 `config.KEYPAD_LAYOUT`, 메뉴 동작 매핑은 `config.KEYPAD_MENU_ACTIONS`(예: 2=위,
  8=아래, 5=확인, D=뒤로)에서 바꿉니다. 두더지 잡기는 OLED 4×4 격자에 각 칸의 키 라벨을
  표시하므로 어느 키를 누를지 바로 보입니다.
