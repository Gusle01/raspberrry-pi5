...
> ⚠️ **7세그(HSN-3643AS)는 LED 3핀(5·6·13)을 자릿수 공통선으로 재사용**한다. 둘은 같은
> 핀이라 동시 사용 불가 — `RASPET_7SEG=0`이면 LED 모드, 기본(=1)이면 7세그 모드로 자동
...
- **끄기/LED와 교체:** `RASPET_7SEG=0`이면 7세그를 끄고 GPIO5·6·13을 다시 **LED 3개**로 쓴다.
...
- ⚠️ **7세그먼트(3-4)를 쓰면 LED 핀(5·6·13)이 자릿수 공통선으로 넘어갑니다.** 기본값
  `RASPET_7SEG=1`에선 LED가 더미(무동작)가 되고, 두더지 잡기의 LED 표시등은 OLED 화면으로
  대체됩니다. LED를 쓰려면 `RASPET_7SEG=0`으로 실행하세요.
